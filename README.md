...
## Ход работы:
1. Открытие редактора crontab. Чтобы добавить новую задачу в cron, используем команду crontab с ключом -e для редактирования:

crontab -e
Если это первый раз, когда вы редактируете crontab, система предложит выбрать текстовый редактор. Выберите тот, которым вам удобнее пользоваться (например, nano, vim).
